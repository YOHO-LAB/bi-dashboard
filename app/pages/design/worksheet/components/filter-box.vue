        ...
        <transition name="filter-box" @beforeEnter="beforeEnter" @afterLeave="afterLeave">
            <div class="filter-box-container" v-show="visiable">
                ...
                <div class="filter-box-list">
                    <component
                        v-for="(filter, index) in worksheet.filters"
                        :key="index"
                        class="filter-item"
                        :is="`FilterBox${filter.options.filterType}`"
                        :value="filter"
                        :list="worksheet.viewData.filterEnums[filter.title]"
...
export default {
  ...
  computed: {
    ...mapState(['worksheet']),
    show() {
      return this.worksheet.filters.length && !_.isEmpty(this.worksheet.viewData);
    }
  },
  data() {
    return {
  ...
  methods: {
    ...
    filterChange({value, val}) {
      this.$store.dispatch(FILTER_CHANGE, {value, val});
    }
  },
};
</script>
