  ...
  let saveAsImage = function(canvas, width, height, type) {
    if ($support.canvas && $support.dataURL) {
      ...
      if (/bmp/.test(type)) {
        ...
        return saveFile(makeURI(strData, downloadMime));
      } else {
        let strData = getDataURL(canvas, type, width, height);

        return saveFile(strData.replace(type, downloadMime));
      }
    }
  };
...
const capture = async(dom, width, height) => {
  const canvasObj = await html2canvas(dom);

  return Canvas2Image.saveAsPNG(canvasObj, width, height);
};
